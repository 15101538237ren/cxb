...
var $dp,WdatePicker;(function(){
	...
	switch(navigator.appName){
		...
		case"Opera":b=true;break;default:I=true;break
	}
	A=L();
		...
		obj={
			...
			$DV:function(_,$){
				if(_!=""){
					...
					if($)for(var A in $){
						if(this.dt[A]===undefined)this.errMsg="invalid property:"+A;this.dt[A]+=$[A]
					}
	...
	function U(E,_){
		$dp.win=X;B();E=E||{};
		if(_){
			if(!D()){$=$||setInterval(function(){if(V[O].readyState=="complete")clearInterval($);U(null,true)},50);return}
			if($dp.status==0){$dp.status=1;K({el:{innerHTML:""}},true)}
			else return
		}else if(E.eCont){
			E.eCont=$dp.$(E.eCont);$dp.flatCfgs.push(E);
			if($dp.status==2)H()
		}else{
	...
	function K(H,$){
		for(var D in _)if(D.substring(0,1)!="$")$dp[D]=_[D];for(D in H)if($dp[D]!==undefined)$dp[D]=H[D];var E=$dp.el?$dp.el.nodeName:"INPUT";if($||$dp.eCont||new RegExp(/input|textarea|div|span|p|a/ig).test(E))$dp.elProp=E=="INPUT"?"value":"innerHTML";else return;if($dp.lang=="auto")$dp.lang=T?navigator.browserLanguage.toLowerCase():navigator.language.toLowerCase();if(!$dp.dd||$dp.eCont||($dp.lang&&$dp.realLang&&$dp.realLang.name!=$dp.lang&&$dp.getLangIndex&&$dp.getLangIndex($dp.lang)>=0)){if($dp.dd&&!$dp.eCont)V[O].body.removeChild($dp.dd);
		if(_.$dpPath=="")F(A);
	...
	function C(){
		...
		if(isNaN(B)){
			...
			else F+=A.left+$.leftM;$dp.dd.style.top=B+"px";$dp.dd.style.left=F+"px"
		}
	}
})()